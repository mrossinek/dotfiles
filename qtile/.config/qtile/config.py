...
keys = [
    ...
    Key(
        [mod],
        "c",
        lazy.spawn("rofi -modi calc -show calc -no-show-match -no-sort"),
        desc="calculator",
    ),
    Key(
        [mod],
        "d",
        lazy.spawn(
            "rofi -modi json-dict -show json-dict -json-dict-path "
            + expanduser("~/.local/share/dicts/json")
            + " -json-dict-mode 2"
        ),
        desc="dictionary",
    ),
    ...
    Key(
        ...
        "y",
        lazy.spawn(
            'bash -c "feh --bg-fill --randomize '
            + expanduser("~/Pictures/Wallpapers/*")
            + '"'
        ),
        desc="background",
    ),
                ...
                widget.WidgetBox(
                    ...
                    widgets=[
                        ...
                        widget.ThermalSensor(
                            tag_sensor="Package id 0", update_interval=5
                        ),
                        widget.CPUGraph(
                            samples=30,
                            type="line",
