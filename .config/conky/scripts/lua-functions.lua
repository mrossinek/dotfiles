...
function print_icon ( cr, icon, size, xpos, ypos, red, green, blue, alpha )
    ...
    cairo_select_font_face ( cr, "Font Awesome 5 Free", CAIRO_FONT_SLANT_NORMAL, CAIRO_FONT_WEIGHT_BOLD )
...
function annotate_hexagon ( cr, corner, origin_x, origin_y, edge, x_end, red, green, blue, alpha )
    local fac = 6 / 5
    local height = math.sqrt(3) * edge / 2
    if corner == 1 then
        cairo_move_to ( cr, origin_x - edge, origin_y )
...
function fill_hexagon ( cr, corner, xpos, ypos, edge, percent, red, green, blue, alpha )
    origin_x, origin_y = compute_hexagon_origin ( corner, xpos, ypos, edge )
    local height = math.sqrt(3) * edge / 2
    ...
    if percent >= 50 then
        ...
        cairo_line_to ( cr, origin_x - edge, origin_y )
        cairo_line_to ( cr, origin_x - (edge - x_offset), origin_y - height * (percent-50) / 50 )
        cairo_line_to ( cr, origin_x + (edge - x_offset), origin_y - height * (percent-50) / 50 )
        cairo_line_to ( cr, origin_x + edge, origin_y )
    else
        x_offset = height * (1 - percent / 50) * math.sqrt(3) / 3
        cairo_line_to ( cr, origin_x - (edge - x_offset), origin_y + height * (1 - percent / 50) )
        cairo_line_to ( cr, origin_x + (edge - x_offset), origin_y + height * (1 - percent / 50) )
    end
    cairo_line_to ( cr, origin_x + edge / 2, origin_y + height )
    cairo_close_path ( cr )
...
function conky_laptop ( cr )
    ...
    volume = tonumber ( conky_parse ( "${exec pactl list sinks | grep Volume | grep -o '[0-9]*%' | head -1 | sed 's/%//'}" ) )
...
function conky_networking ( cr )
    ...
    net = conky_parse ( "${if_up enp0s31f6}enp0s31f6${else}${if_up wlp3s0}wlp3s0${endif}${endif}" )
    ...
    elseif net == 'wlp3s0' then
        ...
        upload = conky_parse ("${upspeed " .. net .. "}")
        download = conky_parse ("${downspeed " .. net .. "}")
        wifi_qual = tonumber ( conky_parse ( "${wireless_link_qual_perc " .. net .. "}" ) )
...
function conky_cpu ( cr )
    ...
    cpu_corner_3_x, cpu_corner_3_y = cpu_origin_x + cpu_edge / 2, cpu_origin_y - cpu_height
    ...
    fill_hexagon ( cr, 6, cpu_origin_x - cpu_edge / 2, cpu_origin_y - cpu_height, 30, cpu_temp, 0.35, 0.35, 0.35, 1 )
    draw_hexagon ( cr, 6, cpu_origin_x - cpu_edge / 2, cpu_origin_y - cpu_height, 30, 0.7, 0.7, 0.7, 1 )
    draw_hexagon ( cr, 4, cpu_origin_x - cpu_edge / 2, cpu_origin_y - cpu_height - 52, 20, 0.7, 0.7, 0.7, 1 )
    ...
    fill_hexagon ( cr, 5, cpu_corner_3_x, cpu_corner_3_y, thread_edge, cpu_perc_1, 0.35, 0.35, 0.35, 1 )
    draw_hexagon ( cr, 5, cpu_corner_3_x, cpu_corner_3_y, thread_edge, 0.7, 0.7, 0.7, 1 )

    cpu_perc_2 = tonumber ( conky_parse ( "${cpu cpu2}" ) )
    fill_hexagon ( cr, 6, cpu_corner_3_x + thread_edge / 2, cpu_corner_3_y - thread_height, thread_edge, cpu_perc_2, 0.35, 0.35, 0.35, 1 )
    draw_hexagon ( cr, 6, cpu_corner_3_x + thread_edge / 2, cpu_corner_3_y - thread_height, thread_edge, 0.7, 0.7, 0.7, 1 )

    cpu_perc_3 = tonumber ( conky_parse ( "${cpu cpu3}" ) )
    fill_hexagon ( cr, 1, cpu_corner_3_x, cpu_corner_3_y, thread_edge, cpu_perc_3, 0.35, 0.35, 0.35, 1 )
    draw_hexagon ( cr, 1, cpu_corner_3_x, cpu_corner_3_y, thread_edge, 0.7, 0.7, 0.7, 1 )

    cpu_perc_4 = tonumber ( conky_parse ( "${cpu cpu4}" ) )
    fill_hexagon ( cr, 6, cpu_corner_3_x + 2 * thread_edge, cpu_corner_3_y, thread_edge, cpu_perc_4, 0.35, 0.35, 0.35, 1 )
    draw_hexagon ( cr, 6, cpu_corner_3_x + 2 * thread_edge, cpu_corner_3_y, thread_edge, 0.7, 0.7, 0.7, 1 )

    cpu_perc_5 = tonumber ( conky_parse ( "${cpu cpu5}" ) )
    fill_hexagon ( cr, 6, cpu_corner_4_x, cpu_corner_4_y, thread_edge, cpu_perc_5, 0.35, 0.35, 0.35, 1 )
    draw_hexagon ( cr, 6, cpu_corner_4_x, cpu_corner_4_y, thread_edge, 0.7, 0.7, 0.7, 1 )

    cpu_perc_6 = tonumber ( conky_parse ( "${cpu cpu6}" ) )
    fill_hexagon ( cr, 1, cpu_corner_4_x + thread_edge, cpu_corner_4_y - 2 * thread_height, thread_edge, cpu_perc_6, 0.35, 0.35, 0.35, 1 )
    draw_hexagon ( cr, 1, cpu_corner_4_x + thread_edge, cpu_corner_4_y - 2 * thread_height, thread_edge, 0.7, 0.7, 0.7, 1 )

    cpu_perc_7 = tonumber ( conky_parse ( "${cpu cpu7}" ) )
    fill_hexagon ( cr, 2, cpu_corner_4_x, cpu_corner_4_y, thread_edge, cpu_perc_7, 0.35, 0.35, 0.35, 1 )
    draw_hexagon ( cr, 2, cpu_corner_4_x, cpu_corner_4_y, thread_edge, 0.7, 0.7, 0.7, 1 )

    cpu_perc_8 = tonumber ( conky_parse ( "${cpu cpu8}" ) )
    fill_hexagon ( cr, 1, cpu_corner_4_x + thread_edge, cpu_corner_4_y, thread_edge, cpu_perc_8, 0.35, 0.35, 0.35, 1 )
    draw_hexagon ( cr, 1, cpu_corner_4_x + thread_edge, cpu_corner_4_y, thread_edge, 0.7, 0.7, 0.7, 1 )
end
