...
function conky_networking(cr)
    ...
    draw_hexagon(cr, 0, 90, 170, 60, 6, 0.7, 0.7, 0.7, 1)
    if net == '' then
        net_icon = ""
    ...
    elseif net == 'wlp3s0' then
        ...
        fill_hexagon(cr, 2, 60, 222, 40, 2, wifi_qual, "%",
                     ...
                     1)
        draw_hexagon(cr, 2, 60, 222, 40, 2, 0.7, 0.7, 0.7, 1)
        draw_hexagon(cr, 1, 100, 290, 20, 1, 0.7, 0.7, 0.7, 1)
    ...
    if vpn_icon ~= '' then
        draw_hexagon(cr, 6, 60, 118, 40, 2, 0.7, 0.7, 0.7, 1)
...
function conky_gpu(cr)
    ...
    draw_hexagon(cr, 0, origin_x, origin_y, edge, 6, 0.7, 0.7, 0.7, 1)
...
function conky_cpu(cr)
    ...
    draw_hexagon(cr, 4, cpu_origin_x - cpu_edge / 2,
                 cpu_origin_y - cpu_height - 52, 20, 1, 0.7, 0.7, 0.7, 1)
    ...
    fill_hexagon(cr, 5, cpu_corner_3_x, cpu_corner_3_y, thread_edge, 2,
                 cpu_perc_1, "%", "MesloLGS Nerd Font Mono", "normal", 20, 0.35,
                 0.35, 0.35, 1)
    draw_hexagon(cr, 5, cpu_corner_3_x, cpu_corner_3_y, thread_edge, 2, 0.7,
                 0.7, 0.7, 1)

    cpu_perc_2 = tonumber(conky_parse("${cpu cpu2}"))
    fill_hexagon(cr, 6, cpu_corner_3_x + thread_edge / 2,
                 cpu_corner_3_y - thread_height, thread_edge, 2, cpu_perc_2,
                 "%", "MesloLGS Nerd Font Mono", "normal", 20, 0.35, 0.35, 0.35,
                 1)
    draw_hexagon(cr, 6, cpu_corner_3_x + thread_edge / 2,
                 cpu_corner_3_y - thread_height, thread_edge, 2, 0.7, 0.7, 0.7,
                 1)

    cpu_perc_3 = tonumber(conky_parse("${cpu cpu3}"))
    fill_hexagon(cr, 1, cpu_corner_3_x, cpu_corner_3_y, thread_edge, 2,
                 cpu_perc_3, "%", "MesloLGS Nerd Font Mono", "normal", 20, 0.35,
                 0.35, 0.35, 1)
    draw_hexagon(cr, 1, cpu_corner_3_x, cpu_corner_3_y, thread_edge, 2, 0.7,
                 0.7, 0.7, 1)

    cpu_perc_4 = tonumber(conky_parse("${cpu cpu4}"))
    fill_hexagon(cr, 6, cpu_corner_3_x + 2 * thread_edge, cpu_corner_3_y,
                 thread_edge, 2, cpu_perc_4, "%", "MesloLGS Nerd Font Mono",
                 "normal", 20, 0.35, 0.35, 0.35, 1)
    draw_hexagon(cr, 6, cpu_corner_3_x + 2 * thread_edge, cpu_corner_3_y,
                 thread_edge, 2, 0.7, 0.7, 0.7, 1)

    cpu_perc_5 = tonumber(conky_parse("${cpu cpu5}"))
    fill_hexagon(cr, 6, cpu_corner_4_x, cpu_corner_4_y, thread_edge, 2,
                 cpu_perc_5, "%", "MesloLGS Nerd Font Mono", "normal", 20, 0.35,
                 0.35, 0.35, 1)
    draw_hexagon(cr, 6, cpu_corner_4_x, cpu_corner_4_y, thread_edge, 2, 0.7,
                 0.7, 0.7, 1)

    cpu_perc_6 = tonumber(conky_parse("${cpu cpu6}"))
    fill_hexagon(cr, 1, cpu_corner_4_x + thread_edge,
                 cpu_corner_4_y - 2 * thread_height, thread_edge, 2, cpu_perc_6,
                 "%", "MesloLGS Nerd Font Mono", "normal", 20, 0.35, 0.35, 0.35,
                 1)
    draw_hexagon(cr, 1, cpu_corner_4_x + thread_edge,
                 cpu_corner_4_y - 2 * thread_height, thread_edge, 2, 0.7, 0.7,
                 0.7, 1)

    cpu_perc_7 = tonumber(conky_parse("${cpu cpu7}"))
    fill_hexagon(cr, 2, cpu_corner_4_x, cpu_corner_4_y, thread_edge, 2,
                 cpu_perc_7, "%", "MesloLGS Nerd Font Mono", "normal", 20, 0.35,
                 0.35, 0.35, 1)
    draw_hexagon(cr, 2, cpu_corner_4_x, cpu_corner_4_y, thread_edge, 2, 0.7,
                 0.7, 0.7, 1)

    cpu_perc_8 = tonumber(conky_parse("${cpu cpu8}"))
    fill_hexagon(cr, 1, cpu_corner_4_x + thread_edge, cpu_corner_4_y,
                 thread_edge, 2, cpu_perc_8, "%", "MesloLGS Nerd Font Mono",
                 "normal", 20, 0.35, 0.35, 0.35, 1)
    draw_hexagon(cr, 1, cpu_corner_4_x + thread_edge, cpu_corner_4_y,
                 thread_edge, 2, 0.7, 0.7, 0.7, 1)

    draw_hexagon(cr, 0, cpu_origin_x, cpu_origin_y, cpu_edge, 6, 0.7, 0.7, 0.7,
                 1)
